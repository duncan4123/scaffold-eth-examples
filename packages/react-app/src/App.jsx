...
const scaffoldEthProvider = new StaticJsonRpcProvider("https://canto.slingshot.finance/")
const mainnetInfura = new StaticJsonRpcProvider("https://canto.slingshot.finance/")
...
function App (props) {
  ...
  for (let c in yourCollectibles) {
    ...
    yourCollectiblesRender.push(
      <Card
        actions={cardActions}
        style={{ backgroundColor: "#eeeeee", border: "1px solid #444444" }}
        key={"your" + yourCollectibles[c].entropy + yourCollectibles[c].id}
        title={<span style={{ color: "#666666" }}>{yourCollectibles[c].name}</span>}
      >
  ...
  const stackedNFTDisplay =
    yourNFTBalance && yourNFTBalance.toNumber() ? (
      <div style={{ maxWidth: 1020, margin: "auto", marginTop: 64, paddingBottom: 256 }}>
        ...
        <StackGrid columnWidth={220} gutterWidth={16} gutterHeight={32}>
        ...
        <div style={{ fontSize: 16, marginTop: 64, opacity: 0.9 }}>
          ...
          <Address
            fontSize={16}
            minimized={false}
            address={nftContractRead && nftContractRead.address}
